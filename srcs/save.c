...
/*   Updated: 2018/05/31 18:03:25 by arusso           ###   ########.fr       */
	/*                                                                            */
/* ************************************************************************** */
...
void	draw_pos(t_global *g, t_local *ex)
{
	...
	while (ex->i < ex->dx)
	{
		...
		if ((ex->x < WIDTH && ex->x >= 0) && (ex->y < HEIGHT && ex->y >= 0))
			mlx_pixel_put(g->mlx, g->win, ex->x + g->indx + ex->decalage, ex->y + g->indy, RED);
		ex->i++;
	}
}
...
void	draw_lines(t_global *g, t_local *ex)
{
	...
	if (!i)
		i = 0;
	if (ex->x0 / g->pas != g->nb_cols - 1)
	{
		i++;
		printf("i = %d\n", i);
		ex->x1 = ex->x0 + g->pas;
		ex->y1 = ex->y0 - g->map[ex->y0 / g->pas][ex->x0 / g->pas + 1];
		printf(" enter : %d %d %d \n", ex->y0, ex->x0, g->pas);
		draw_one_line(g, ex);
		ft_putendl("End enter");
	}
	ex->x0 = tmp_x;
	ex->y0 = tmp_y;
	if (ex->y0 / g->pas != g->nb_lines - 1)
	{
		i++;
		printf("i = %d\n", i);
		ex->x1 = ex->x0 + g->d;
		ex->y1 = ex->y0 + g->pas - g->map[ex->y0 / g->pas + 1][ex->x0 / g->pas];
		printf(" exit : %d %d %d \n\n", ex->y0, ex->x0, g->pas);
		draw_one_line(g, ex);
		ft_putendl("End exit");
	}
...
void	draw_map(t_global *g)
{
	...
	while (l.x0 / g->pas < g->nb_cols - 1 || l.y0 / g->pas < g->nb_lines - 1)
	{
		...
		l.y0 = tmp_y - g->map[l.y0 / g->pas][l.x0 / g->pas];
		printf("in : %d %d %d \n", l.y0, l.x0, g->pas);
		draw_lines(g, &l);
		l.x0 += g->pas;
	}
